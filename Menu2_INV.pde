...
void subMenu2_1(float xBound, float yBound)
{
    ...
    {
        text(Ranged_Weapons.get(i).name +" ("+ Ranged_Weapons.get(i).qty+")", xBound + width*(.075), yBound + height*(.175+(.05*i)));
    }//end for
    for(int i=0; i<Melee_Weapons.size(); i++)
    {
        text(Melee_Weapons.get(i).name +" ("+ Melee_Weapons.get(i).qty+")", xBound + width*(.075), yBound + height*(.175+(.05*(i+Ranged_Weapons.size()))));
    }//end for
    
    if(curEquipW != -1)
    {
        rect(xBound + width*(.04), yBound + height*(.16+(.05*curEquipW)), width*(.02), height*(.028571428));
    }//end if
    ...
    if(subNav-1 < Ranged_Weapons.size())
    {
        ...
        if(Ranged_Weapons.get(subNav-1).type.equals("R"))
        {
            fill(0, 100, 0);
